...
public class RegistrationService {
    ...
    public String register(RegistrationRequest request) {
        ...
        String token = userServiceObj.signUpUser(
        new AppUser(
                request.getEmail(),
                request.getPassword(),
                userRole.USER)
...
//        );
        ...
        String link = "http://localhost:8080/levin/api/confirm?token=" + token;

        String email = request.getEmail();
        int index = email.indexOf('@');
        emailSender.send(email, buildEmail(email.substring(0,index), link));
//        return token;
